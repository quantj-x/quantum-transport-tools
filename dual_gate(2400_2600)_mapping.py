# -*- coding: utf-8 -*-
"""
2400/2600 Dual-Gate Quantum Transport Mapping  —  v1
============================================
Author: Xie Jiao  <xequalux@gmail.com>
Data: 2026-04-15  (Keithley SMU gate sources)

------------------------------------------------------
Hardware setup this build is configured for
-------------------------------------------
GATE SIDE — SMU-driven:

    Keithley unit #1 (top gate)              Keithley unit #2 (bot gate)
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━              ━━━━━━━━━━━━━━━━━━━━━━━━━━━
    source.func = VOLTAGE                    source.func = VOLTAGE
    sense.func  = CURRENT                    sense.func  = CURRENT
    compliance  = 100 nA (default)           compliance  = 100 nA (default)
    output      = ON  during scan            output      = ON  during scan
                  OFF before/after                         OFF before/after
                          │                                         │
                          ▼                                         ▼
                  device top gate                          device bottom gate
...
CRITICAL OPERATOR CHECKLIST (verify each cooldown):
  [ ] BOTH Keithley front panels: OUTPUT OFF before START
  [ ] BOTH Keithley compliance currents set for THIS sample
  [ ] BOTH Keithley VISA addresses reachable (Test button)
...
# Globals & constants
# =====================================================================
APP_NAME       = "2400/2600 Dual-Gate Quantum Transport Mapping"
APP_VERSION    = "v1.0"
ORG_NAME       = "lab.transport"
SETTINGS_NAME  = "dual_gate_mapping_v1"  # bumped: gate hardware changed
                                          # vs DAQ, GUI fields are not
                                          # backward-compatible.
...
# DAQ defaults — AI side only.  Gates are now driven by Keithley SMUs.
...
# ---- v2.4: Keithley SMU options ----
SMU_MODEL_K2400 = 'K2400'
SMU_MODEL_K2636 = 'K2636'
SMU_MODEL_OPTIONS = (SMU_MODEL_K2400, SMU_MODEL_K2636)

# Default compliance current for hBN-gated devices.  Conservative;
# operator MUST verify this is appropriate for the specific sample
# BEFORE pressing START.
SMU_DEFAULT_COMPLIANCE_A = 100e-9    # 100 nA

# Software ramp parameters.  In v2.4, the SMU does the actual sourcing,
# but we still ramp in software (in 20-mV steps at 2 V/s) so that we
# can do compliance monitoring after every step and abort cleanly if
# leakage exceeds the limit at any intermediate voltage.
SMU_RAMP_RATE_V_PER_S = 2.0
SMU_RAMP_STEP_V       = 0.02

# Software-side compliance abort threshold.  Setting < 1.0 means we
# abort BEFORE the SMU itself enters compliance (the hardware limit),
# giving us a clean error message with the exact gate voltage that
# caused it instead of waiting for the SMU to clamp.
SMU_COMPLIANCE_ABORT_FRACTION = 0.95
...
# v2.4: Same gating pattern for PyVISA (Keithley SMU communication).
pyvisa = None
...
# =====================================================================
# 1b. Keithley SMU gate source (v2.4)
# =====================================================================
class ComplianceTrippedError(RuntimeError):
    """Raised when a Keithley SMU's measured current exceeds the
    software-side compliance abort fraction.  The MeasurementThread
    catches this, ramps both gates safely back to 0 V, opens output,
    and reports which gate / which voltage tripped."""
...
class KeithleyGateSource:
    """Drives one device gate from one Keithley SMU channel.

    Designed to be a drop-in replacement for the AO half of DAQHardware,
    bundled together via CompositeHardware below.  Each instance owns
    ONE physical SMU channel (K2400 has only one channel; K2636 we
    use SMU 'a' by default but the channel is configurable).

    Compliance current is enforced both by the SMU itself (hardware
    via :SENS:CURR:PROT for K2400 / source.limiti for K2636) AND
    verified in software after every voltage step.  If the measured
    current exceeds compliance × SMU_COMPLIANCE_ABORT_FRACTION,
    `write_voltage()` raises ComplianceTrippedError so the upstream
    sweep loop can abort cleanly.

    The class is import-safe in DEMO_MODE (no PyVISA needed): in
    demo mode all SMU operations are stubs that update an in-memory
    state and synthesise a tiny ~1 pA leakage current for realism.
    """

    def __init__(self, label, model, address, channel='a',
                 compliance_a=SMU_DEFAULT_COMPLIANCE_A,
                 voltage_range_v=10.0):
        """
        label:           'top' or 'bot' (used in error messages and metadata)
        model:           SMU_MODEL_K2400 or SMU_MODEL_K2636
        address:         VISA resource string, e.g. 'GPIB0::24::INSTR'
                         or 'TCPIP0::192.168.1.10::inst0::INSTR'
        channel:         only used for K2636: 'a' or 'b' (default 'a')
        compliance_a:    current limit in amps (hardware-enforced)
        voltage_range_v: SMU source range; pick the smallest range that
                         covers your max gate voltage.  Default 10 V.
        """
        if model not in SMU_MODEL_OPTIONS:
            raise ValueError(f"Unknown SMU model {model!r}; expected "
                             f"one of {SMU_MODEL_OPTIONS}")
    ...
    # ---- low-level SCPI/TSP ----
    def _write(self, cmd):
        if DEMO_MODE or self._inst is None:
            return
    ...
    # ---- lifecycle ----
    def open(self):
        """Open VISA session and configure SMU in voltage-source mode
        with output OFF and source level = 0.  Output is NOT turned on
        here — that happens in arm_output() right before scanning starts.
        """
        if DEMO_MODE:
            self.connected = True
            self._idn = f"DEMO {self.model} {self.label}"
            return
        rm = pyvisa.ResourceManager()
        self._inst = rm.open_resource(self.address)
        self._inst.timeout = 5000
        # Don't *RST — some firmware versions glitch the output briefly
        # on reset.  We instead carefully configure each setting from
        # whatever state the SMU was already in.
        try:
            self._idn = self._inst.query('*IDN?').strip()
        except Exception as e:
            raise RuntimeError(
                f"Failed to query *IDN? on {self.label} SMU at "
                f"{self.address!r}: {e}.  Check VISA address and cabling.")

        if self.model == SMU_MODEL_K2400:
            self._configure_k2400()
        else:
            self._configure_k2636()
        self.connected = True

    def _configure_k2400(self):
        """K2400 SCPI configuration sequence.

        Order matters: configure source / sense / compliance BEFORE
        turning output on, so we never have a window where the SMU is
        sourcing an unknown voltage with no compliance limit.
        """
        # Make sure output is OFF first.
        self._write(':OUTP OFF')
        # Source = voltage, fixed mode (not sweep)
        self._write(':SOUR:FUNC VOLT')
        self._write(':SOUR:VOLT:MODE FIXED')
        self._write(f':SOUR:VOLT:RANG {self.voltage_range_v}')
        self._write(':SOUR:VOLT:LEV 0')
        # Sense current; range covers compliance with 10× margin
        self._write(':SENS:FUNC "CURR"')
        sens_range = max(self.compliance_a * 10, 1e-9)
        self._write(f':SENS:CURR:RANG {sens_range}')
        # Hardware-enforced compliance limit
        self._write(f':SENS:CURR:PROT {self.compliance_a}')
        # 4-wire OFF (we're sourcing V, not measuring R)
        self._write(':SYST:RSEN OFF')
        # NPLC = 1 → 16-20 ms per current read at 50/60 Hz line freq
        self._write(':SENS:CURR:NPLC 1')
        self._write(':SYST:AZER ON')
        # Format: read CURRent only (we already know V from set-point)
        self._write(':FORM:ELEM CURR')

    def _configure_k2636(self):
        """K2636B TSP configuration sequence (note: TSP, not SCPI)."""
        ch = f'smu{self.channel}'
        self._write(f'{ch}.source.output = {ch}.OUTPUT_OFF')
        self._write(f'{ch}.source.func = {ch}.OUTPUT_DCVOLTS')
        self._write(f'{ch}.source.autorangev = {ch}.AUTORANGE_OFF')
        self._write(f'{ch}.source.rangev = {self.voltage_range_v}')
        self._write(f'{ch}.source.levelv = 0')
        # Hardware-enforced compliance limit
        self._write(f'{ch}.source.limiti = {self.compliance_a}')
        # Sense current explicitly with range
        sens_range = max(self.compliance_a * 10, 1e-9)
        self._write(f'{ch}.measure.autorangei = {ch}.AUTORANGE_OFF')
        self._write(f'{ch}.measure.rangei = {sens_range}')
        self._write(f'{ch}.measure.nplc = 1')
        self._write(f'{ch}.sense = {ch}.SENSE_LOCAL')

    def arm_output(self):
        """Turn output ON.  Must be called AFTER open() and BEFORE
        any write_voltage() call.  Source level should already be 0 V
        from open()'s configuration step.
        """
        if DEMO_MODE:
            return
        if self.model == SMU_MODEL_K2400:
            ...
            ch = f'smu{self.channel}'
            self._write(f'{ch}.source.output = {ch}.OUTPUT_ON')

    def disarm_output(self):
        """Turn output OFF.  Should be called after the scan ends and
        the source level has been ramped to 0.  Safe to call multiple
        times.
        """
        if DEMO_MODE:
            return
    ...
    def close(self):
        """Disarm + close VISA session.  Safe to call multiple times."""
        try:
            self.disarm_output()
        except Exception:
            pass
        if not DEMO_MODE and self._inst is not None:
            try:
                self._inst.close()
            except Exception:
                pass
    ...
    # ---- voltage write + compliance check ----
    def write_voltage(self, v):
        """Write source voltage AND verify compliance.

        Steps:
          1. Clip to ±voltage_range_v (defensive — the upstream
             VoltageLimits should already have caught any out-of-range)
          2. Send the source level command
          3. Read measured current
          4. If |I| > compliance × SMU_COMPLIANCE_ABORT_FRACTION,
             raise ComplianceTrippedError.

        Returns: measured current in amps.
        """
        v = float(np.clip(v, -self.voltage_range_v, self.voltage_range_v))

        if DEMO_MODE:
            self._last_v = v
            # Synthesize a small leakage current for demo realism
            i = 1e-12 * v + 0.5e-12 * np.random.randn()
            self._last_i = float(i)
            return self._last_i

        if self.model == SMU_MODEL_K2400:
            self._write(f':SOUR:VOLT:LEV {v}')
            i_str = self._query(':READ?')
        else:
            ch = f'smu{self.channel}'
            self._write(f'{ch}.source.levelv = {v}')
            i_str = self._query(f'print({ch}.measure.i())')

        try:
            i = float(i_str)
        except (ValueError, TypeError):
            raise RuntimeError(
                f"{self.label} SMU returned non-numeric current "
                f"reading: {i_str!r}")

        self._last_v = v
        self._last_i = i

        abort_thresh = self.compliance_a * SMU_COMPLIANCE_ABORT_FRACTION
        if abs(i) > abort_thresh:
            raise ComplianceTrippedError(
                self.label, v, i, self.compliance_a)
        return i

    def get_last(self):
        """Return (last_v, last_i) without doing a new SMU read."""
        return self._last_v, self._last_i


# =====================================================================
# 1c. Composite hardware wrapper (v2.4)
# =====================================================================
class CompositeHardware:
    """Bundles two KeithleyGateSource instances (top and bot gates) with
    one DAQHardware (AI readout only) and exposes the same interface
    that MeasurementThread already speaks:

        ao_chans         — symbolic names for the gate channels
        ai_chans         — DAQ AI channel list
        connected        — True iff all sub-devices are connected
        ai_terminal_name — proxied from DAQ
        ai_sample_rate   — proxied from DAQ
        n_avg            — proxied from DAQ
        open()           — opens DAQ, opens both SMUs, arms outputs
        close()          — disarms, closes everything
        write_ao(ch, v)  — routes to top or bot SMU, monitors compliance
        ramp_ao(ch, v)   — soft ramp via repeated write_voltage calls
        read_ai()        — proxied to DAQ
        set_n_avg(n)     — proxied to DAQ

    The "channel" passed to write_ao / ramp_ao is now the symbolic
    name 'top' or 'bot', NOT a DAQ channel string.  Up the call stack,
    parse_params and run() use the same 'top'/'bot' tokens.
    """
    ...
    def __init__(self,
                 top_smu: 'KeithleyGateSource',
                 bot_smu: 'KeithleyGateSource',
                 daq: 'DAQHardware'):
        self.top_smu = top_smu
        self.bot_smu = bot_smu
        self.daq = daq
        self.ao_chans = [self.AO_TOP, self.AO_BOT]
        self.ai_chans = list(daq.ai_chans)
        self.connected = False
        # Mirror DAQ attributes for code that introspects them
        self.ai_terminal_name = daq.ai_terminal_name
        self.ai_sample_rate = daq.ai_sample_rate
        # Last-read compliance currents, refreshed by every write_ao /
        # ramp_ao call.  Used by MeasurementThread to write the
        # I_top_compliance / I_bot_compliance CSV columns.
        self._last_compliance_top = 0.0
        self._last_compliance_bot = 0.0

    @property
    def n_avg(self):
        return self.daq.n_avg

    @n_avg.setter
    def n_avg(self, value):
        self.daq.n_avg = value

    def set_n_avg(self, n):
        self.daq.set_n_avg(n)

    # ---- lifecycle ----
    def open(self):
        """Open all sub-devices.  Order matters:

          1. Open DAQ first (cheap, can't damage anything)
          2. Open top SMU (configures source/sense/compliance, output OFF)
          3. Open bot SMU (same)
          4. Arm both SMU outputs (gates physically driven, but at 0 V)

        If step 2 or 3 fails, we close any already-opened sub-device
        before re-raising, so callers don't end up with a half-open
        composite.
        """
        try:
            self.daq.open()
        ...
        try:
            ...
            try: self.top_smu.close()
            except Exception: pass
            try: self.daq.close()
            except Exception: pass
            raise
        # Both SMUs now configured (output OFF, level 0).  Arm them.
        try:
            self.top_smu.arm_output()
            self.bot_smu.arm_output()
        except Exception:
            try: self.bot_smu.close()
            except Exception: pass
            try: self.top_smu.close()
            except Exception: pass
            try: self.daq.close()
    ...
    def close(self):
        """Safe shutdown sequence.  Each step in its own try/except so
        a failure in one doesn't prevent the others.
        """
        # Best-effort ramp to zero before disarming
        try:
            self.ramp_ao(self.AO_TOP, 0.0)
        except Exception:
            pass
        try:
            self.ramp_ao(self.AO_BOT, 0.0)
        except Exception:
            pass
        try:
            self.top_smu.disarm_output()
        except Exception:
            pass
        try:
            self.bot_smu.disarm_output()
        except Exception:
            pass
        try:
            self.top_smu.close()
    ...
    # ---- AO interface (matches DAQHardware) ----
    def _smu_for(self, channel):
        if channel == self.AO_TOP:
            return self.top_smu
        elif channel == self.AO_BOT:
            return self.bot_smu
        else:
            raise ValueError(
                f"Unknown gate channel {channel!r}; expected "
                f"'{self.AO_TOP}' or '{self.AO_BOT}'")

    def write_ao(self, channel, voltage):
        """Single-step write.  Used by the inner sweep loop after a
        successful soft-ramp arrival at the previous fast point — the
        step from one fast point to the next is small enough that we
        don't need to ramp.

        IMPORTANT: this can raise ComplianceTrippedError.  The caller
        (MeasurementThread._sweep_one_direction) catches it; the outer
        try/except in run() handles ramp-down + sidecar write + error
        reporting.
        """
        smu = self._smu_for(channel)
        i = smu.write_voltage(voltage)
        if channel == self.AO_TOP:
            self._last_compliance_top = i if i is not None else 0.0
        else:
            self._last_compliance_bot = i if i is not None else 0.0
        # In demo mode, feed the SMU voltage into the DAQ's internal
        # _last_ao dict so that _demo_read_ai sees varying AO state and
        # produces the CNP-feature pattern that makes demo runs useful
        # for exercising analysis pipelines.  In real hardware mode the
        # DAQ's AO state is unused and this is a cheap no-op.
        if DEMO_MODE and len(self.daq.ao_chans) >= 2:
            idx = 0 if channel == self.AO_TOP else 1
            self.daq._last_ao[self.daq.ao_chans[idx]] = voltage

    def ramp_ao(self, channel, v_target,
                rate_v_per_s=SMU_RAMP_RATE_V_PER_S,
                step=SMU_RAMP_STEP_V):
        """Soft ramp from current voltage to v_target in `step`-sized
        increments at approximately rate_v_per_s.  Each step verifies
        compliance via write_voltage().

        Default rate (2 V/s) and step (20 mV) match the DAQHardware
        ramp behaviour, so the slow-ramp safety properties used by the
        strategy / inner loop are preserved.
        """
        smu = self._smu_for(channel)
        v0, _ = smu.get_last()
        ...
        for v in np.linspace(v0, v_target, n):
            ...
            time.sleep(dwell)
        if channel == self.AO_TOP:
            self._last_compliance_top = smu.get_last()[1]
        else:
            self._last_compliance_bot = smu.get_last()[1]
        # Feed final voltage into DAQ for demo realism (see write_ao).
        if DEMO_MODE and len(self.daq.ao_chans) >= 2:
            idx = 0 if channel == self.AO_TOP else 1
            self.daq._last_ao[self.daq.ao_chans[idx]] = v_target

    # ---- AI interface (proxied to DAQ) ----
    def read_ai(self):
        return self.daq.read_ai()

    # ---- compliance accessors ----
    def get_compliance_currents(self):
        """Return (I_top_last, I_bot_last) in amperes."""
        return self._last_compliance_top, self._last_compliance_bot
...
class MeasurementThread(QThread):
    ...
    def __init__(self, params, hardware):
        """
        params:   dict from QuantumTransportGUI._parse_params()
        hardware: bare DAQHardware instance (AI readout only).  v2.4
                  builds a CompositeHardware at the start of run() that
                  wraps this DAQ + two Keithley SMUs constructed from
                  params['smu_config'].  self.hw is swapped to the
                  composite for the duration of the scan and reverted
                  in finally.
        """
        super().__init__()
        self.p = params
        self._bare_daq = hardware
        self.hw = hardware     # swapped to composite in run()
        ...
        # Symbolic gate channel names: always 'top' / 'bot' in v2.4.
        self.ao_top = params['ao_top']
        self.ao_bot = params['ao_bot']
        self.smu_config = params['smu_config']
        # SMU instances populated in run() after successful open().
        # Saved here so _write_txt_sidecar / _write_metadata_sidecar
        # can access model / IDN / address / compliance in the finally
        # block even if the scan aborts mid-way.
        self._top_smu = None
        self._bot_smu = None
        # Abort cause (populated on compliance trip).  None if scan
        # finished normally or was user-aborted.
    ...
    def _write_csv_header(self, writer):
        ...
        v2.4: two new columns at the end of every row — the per-point
        measured SMU compliance currents:
            I_top_compliance_A, I_bot_compliance_A
        ...
        cols.append('I_top_compliance_A')
        cols.append('I_bot_compliance_A')
    ...
    def _write_txt_sidecar(self):
        ...
        # ---- Gate sources (Keithley SMUs, v2.4) ----
        smu = self.smu_config
        L.append("[ Gate sources  (Keithley SMUs, v2.4) ]")
        L.append(_row("top_model",        smu['top_model']))
        L.append(_row("top_visa_address", smu['top_addr']))
        L.append(_row("top_compliance_a",
                      f"{smu['top_compliance_a']:.3e}  "
                      f"({smu['top_compliance_a']*1e9:.1f} nA)"))
        if self._top_smu is not None and self._top_smu._idn:
            L.append(_row("top_idn", self._top_smu._idn))
        L.append(_row("bot_model",        smu['bot_model']))
        L.append(_row("bot_visa_address", smu['bot_addr']))
        L.append(_row("bot_compliance_a",
                      f"{smu['bot_compliance_a']:.3e}  "
                      f"({smu['bot_compliance_a']*1e9:.1f} nA)"))
        if self._bot_smu is not None and self._bot_smu._idn:
            L.append(_row("bot_idn", self._bot_smu._idn))
        L.append(_row("ramp_rate_v_per_s", SMU_RAMP_RATE_V_PER_S))
        L.append(_row("ramp_step_v",       SMU_RAMP_STEP_V))
        L.append(_row("compliance_abort_fraction",
                      SMU_COMPLIANCE_ABORT_FRACTION))
        if self._abort_cause is not None:
            L.append("")
            L.append("  *** ABORT CAUSE ***")
            for k, v in self._abort_cause.items():
                L.append(_row(k, v, indent=4))
        L.append("")
        ...
        L.append("    Vtg_actual         physical top-gate voltage written to SMU (V)")
        L.append("    Vbg_actual         physical bottom-gate voltage written to SMU (V)")
        ...
        for ch in self.enabled_channels:
            ...
            L.append(f"    {ch.csv_col_name}_std    "
                     ...
                     f"{unit_pretty})")
        L.append("    I_top_compliance_A top-gate SMU measured leakage current (A)")
        L.append("    I_bot_compliance_A bottom-gate SMU measured leakage current (A)")
        L.append("")
        L.append("=" * 78)
    ...
    def _write_metadata_sidecar(self):
        ...
        meta = {
            ...
            'gates_smu': {
                # v2.4: gate channels are now Keithley SMUs, not DAQ AO.
                'ao_top_symbolic': self.ao_top,       # 'top'
                'ao_bot_symbolic': self.ao_bot,       # 'bot'
                'top_model':         self.smu_config['top_model'],
                'top_visa_address':  self.smu_config['top_addr'],
                'top_compliance_a':  self.smu_config['top_compliance_a'],
                'top_idn': (self._top_smu._idn
                            if self._top_smu is not None else None),
                'bot_model':         self.smu_config['bot_model'],
                'bot_visa_address':  self.smu_config['bot_addr'],
                'bot_compliance_a':  self.smu_config['bot_compliance_a'],
                'bot_idn': (self._bot_smu._idn
                            if self._bot_smu is not None else None),
                'ramp_rate_v_per_s':         SMU_RAMP_RATE_V_PER_S,
                'ramp_step_v':               SMU_RAMP_STEP_V,
    ...
    def _emit_skip(self, writer, slow_t, fast_t, j_idx, i_idx, direction):
        ...
        for _ch in self.enabled_channels:
            ...
            data_cols.append(float('nan'))
        # v2.4: skipped rows have NaN for both compliance currents too,
        # since no voltage was written to either SMU at this target.
        row = ([slow_t, fast_t, float('nan'), float('nan'), 1, direction]
               + data_cols
               + [float('nan'), float('nan')])
        writer.writerow(row)
        info = PointInfo(
    ...
    def _emit_valid(self, writer, slow_t, fast_t, Vtg, Vbg,
        ...
        # v2.4: append the last-measured SMU compliance currents.
        # `self.hw` is a CompositeHardware in v2.4; for backward
        # compatibility if it happens to be a bare DAQHardware (should
        # not occur in v2.4 but defensive) we fall back to NaN.
        if hasattr(self.hw, 'get_compliance_currents'):
            i_top, i_bot = self.hw.get_compliance_currents()
        else:
            i_top, i_bot = float('nan'), float('nan')
        row = ([slow_t, fast_t, Vtg, Vbg, 0, direction]
               + data_cols
               + [i_top, i_bot])
        writer.writerow(row)
        info = PointInfo(
    ...
    def run(self):
        ...
        # ----- v2.4: build CompositeHardware for this scan -----
        # The GUI-owned self._bare_daq is the DAQ (AI-only).  Construct
        # two KeithleyGateSource instances from smu_config, bundle with
        # the DAQ, open everything in order.  If any step fails, any
        # already-opened sub-device is torn down cleanly inside
        # CompositeHardware.open().
        smu = self.smu_config
        try:
            top_smu = KeithleyGateSource(
                label='top',
                model=smu['top_model'],
                address=smu['top_addr'],
                compliance_a=smu['top_compliance_a'],
                voltage_range_v=max(self.limits.Vtg_max * 1.1, 10.0),
            )
            bot_smu = KeithleyGateSource(
                label='bot',
                model=smu['bot_model'],
                address=smu['bot_addr'],
                compliance_a=smu['bot_compliance_a'],
                voltage_range_v=max(self.limits.Vbg_max * 1.1, 10.0),
            )
        except Exception as e:
            tb = traceback.format_exc()
            self.log_msg.emit(
                f"FATAL: cannot construct SMU objects: {e}", "error")
            self.error_occurred.emit(tb)
            return
        self._top_smu = top_smu
        self._bot_smu = bot_smu

        # The bare DAQ may or may not already be open (GUI opens it at
        # startup).  close() it first to release the AI task cleanly, so
        # CompositeHardware.open() can re-open it with a fresh task.
        try:
            self._bare_daq.close()
        except Exception:
            pass

        composite = CompositeHardware(top_smu, bot_smu, self._bare_daq)
        try:
            composite.open()
        except Exception as e:
            tb = traceback.format_exc()
            self.log_msg.emit(
                f"FATAL: hardware open failed: {e}.  "
                f"Check VISA addresses and DAQ connection.  "
                f"Aborting scan before any data is written.", "error")
            # Try to reopen the bare DAQ so the GUI self-test works next time
            try: self._bare_daq.open()
            except Exception: pass
            self.error_occurred.emit(tb)
            return

        # Swap self.hw to the composite.  The write_ao / ramp_ao / read_ai
        # / set_n_avg calls throughout this method (and all helper methods)
        # now go through CompositeHardware which routes AO to SMUs and AI
        # to DAQ.
        self.hw = composite

        # Reconfigure DAQ AI timing for this scan's n_avg.
        try:
            ...
            self.log_msg.emit(
                ...
                f"Aborting scan before any data is written.", "error")
            # tear down composite, reopen bare DAQ for next scan
            try: composite.close()
            except Exception: pass
            self.hw = self._bare_daq
            try: self._bare_daq.open()
            except Exception: pass
            self.error_occurred.emit(tb)
            return
        ...
        try:
            ...
            self.log_msg.emit(
                f"Gate sources: top={smu['top_model']}@{smu['top_addr']} "
                f"(compl {smu['top_compliance_a']*1e9:.0f} nA), "
                f"bot={smu['bot_model']}@{smu['bot_addr']} "
                f"(compl {smu['bot_compliance_a']*1e9:.0f} nA).  "
            ...
            self.log_msg.emit(
                ...
                "info")
            if top_smu._idn:
                self.log_msg.emit(f"top SMU IDN: {top_smu._idn}", "info")
            if bot_smu._idn:
                self.log_msg.emit(f"bot SMU IDN: {bot_smu._idn}", "info")
        ...
        except ComplianceTrippedError as ce:
            # Dedicated handler — this is sample-safety critical.  Log
            # explicitly, record the cause, emit error_occurred.  The
            # finally block handles the safe ramp-down via composite.close().
            self._abort_cause = {
                'kind': 'compliance',
                'label': ce.label, 'voltage_v': ce.voltage,
                'current_a': ce.current, 'compliance_a': ce.compliance,
            }
            self.log_msg.emit(
                f"COMPLIANCE TRIPPED on {ce.label} gate at "
                f"V={ce.voltage:+.4f} V: measured I={ce.current:+.3e} A "
                f"(limit {ce.compliance:.3e} A).  Aborting scan NOW.",
                "error")
            tb = traceback.format_exc()
            self.error_occurred.emit(tb)
            ...
            self._abort_cause = {
                'kind': 'exception',
                'traceback_last_line': tb.splitlines()[-1],
            }
            self.error_occurred.emit(tb)
        finally:
            # Safe ramp-down via composite.close() which ramps each SMU
            # to 0 V, disarms output, closes VISA sessions, and closes DAQ.
            try:
                self.log_msg.emit("Ramping gates back to 0 V and closing SMUs...",
                                  "info")
                composite.close()
                self.log_msg.emit(
                    "Gates safely at 0 V, SMU outputs OFF.", "success")
            except Exception as e:
                self.log_msg.emit(f"WARN: SMU tear-down failed: {e}",
                                  "warning")
            # Restore self.hw to the bare DAQ and reopen it so the GUI
            # self-test / idle state still works.
            self.hw = self._bare_daq
            try:
                self._bare_daq.open()
            except Exception as e:
                self.log_msg.emit(
                    f"WARN: could not reopen DAQ after scan: {e}",
                    "warning")
            try:
...
class QuantumTransportGUI(QMainWindow):
    ...
    def _build_gates_group(self):
        """v2.4: SMU-based gate sources.  Each gate has its own
        independent Keithley SMU.  GUI exposes per-gate model selector
        (K2400 vs K2636), VISA address, compliance current, plus a
        Test button to verify connectivity."""
        gb = QGroupBox("Gate sources  (Keithley SMU per gate, v2.4)")
        grid = QGridLayout()
        grid.setContentsMargins(8, 4, 8, 8)
        grid.setHorizontalSpacing(8)
        grid.setVerticalSpacing(6)

        for col, h in enumerate(['Gate', 'Model', 'VISA address',
                                 'Compliance (A)', '']):
            l = QLabel(h)
            l.setObjectName("sectionLabel")
        ...
        # ----- top gate row -----
        grid.addWidget(QLabel("Top (Vtg)"), 1, 0)

        self.cb_top_smu_model = NoWheelComboBox()
        self.cb_top_smu_model.addItems(SMU_MODEL_OPTIONS)
        self.cb_top_smu_model.setCurrentText(SMU_MODEL_K2400)
        self.cb_top_smu_model.setToolTip(
            "Keithley SMU model driving the top gate.\n"
            "  K2400 — single-channel, ±21 V, 1 nA – 1 A\n"
            "  K2636 — dual-channel (we use SMU A only), fA-resolution\n"
            "Both top and bottom can be the same model or mixed.")
        grid.addWidget(self.cb_top_smu_model, 1, 1)

        self.le_top_smu_addr = QLineEdit('GPIB0::24::INSTR')
        self.le_top_smu_addr.setToolTip(
            "VISA resource string for the top SMU.\n"
            "Common formats:\n"
            "  GPIB0::<addr>::INSTR     (GPIB)\n"
            "  TCPIP0::<ip>::inst0::INSTR  (LAN)\n"
            "  USB0::0x05E6::0x2400::<sn>::INSTR  (USB)\n"
            "Use NI-MAX or pyvisa-shell to discover addresses.")
        grid.addWidget(self.le_top_smu_addr, 1, 2)

        self.le_top_smu_compl = QLineEdit(f'{SMU_DEFAULT_COMPLIANCE_A:g}')
        self.le_top_smu_compl.setToolTip(
            "Hardware compliance current (amps).  The SMU will not\n"
            "exceed this current; if leakage approaches it (within 5 %),\n"
            "the scan aborts and ramps both gates to 0 V.\n"
            f"Default {SMU_DEFAULT_COMPLIANCE_A*1e9:g} nA — conservative for\n"
            "hBN-gated devices.  Increase only if you know the sample\n"
            "can tolerate more leakage.")
        grid.addWidget(self.le_top_smu_compl, 1, 3)

        self.btn_test_top_smu = QPushButton("Test")
        self.btn_test_top_smu.setToolTip(
            "Open the top SMU at the address above, query *IDN?, and\n"
            "log the response.  Use this to verify connectivity BEFORE\n"
            "committing to a long scan.  Closes the connection after.")
        self.btn_test_top_smu.clicked.connect(
            lambda: self._test_smu('top'))
        grid.addWidget(self.btn_test_top_smu, 1, 4)

        # ----- bot gate row -----
        grid.addWidget(QLabel("Bottom (Vbg)"), 2, 0)

        self.cb_bot_smu_model = NoWheelComboBox()
        self.cb_bot_smu_model.addItems(SMU_MODEL_OPTIONS)
        self.cb_bot_smu_model.setCurrentText(SMU_MODEL_K2400)
        self.cb_bot_smu_model.setToolTip(
            "Keithley SMU model driving the bottom gate.")
        grid.addWidget(self.cb_bot_smu_model, 2, 1)

        self.le_bot_smu_addr = QLineEdit('GPIB0::25::INSTR')
        self.le_bot_smu_addr.setToolTip(
            "VISA resource string for the bottom SMU.\n"
            "MUST be different from the top SMU address.")
        grid.addWidget(self.le_bot_smu_addr, 2, 2)

        self.le_bot_smu_compl = QLineEdit(f'{SMU_DEFAULT_COMPLIANCE_A:g}')
        self.le_bot_smu_compl.setToolTip(
            "Hardware compliance current for the bottom gate.")
        grid.addWidget(self.le_bot_smu_compl, 2, 3)

        self.btn_test_bot_smu = QPushButton("Test")
        self.btn_test_bot_smu.clicked.connect(
            lambda: self._test_smu('bot'))
        grid.addWidget(self.btn_test_bot_smu, 2, 4)

        # ----- which gate is the FAST (inner) axis in Vtg-Vbg mode -----
        ...
        self.cb_fast_gate.setToolTip(
            ...
            "In n-D mode this is ignored.")
        self.cb_fast_gate.currentIndexChanged.connect(self._refresh_sweep_labels)
        grid.addWidget(self.cb_fast_gate, 3, 1, 1, 4)

        grid.setColumnStretch(2, 1)
        gb.setLayout(grid)
        return gb

    def _test_smu(self, which):
        """Quick *IDN? check on the configured SMU.  Opens the VISA
        session, reads IDN, closes.  Logs result to event log."""
        if which == 'top':
            model = self.cb_top_smu_model.currentText()
            addr  = self.le_top_smu_addr.text().strip()
            try:
                compl = float(self.le_top_smu_compl.text())
            except ValueError:
                compl = SMU_DEFAULT_COMPLIANCE_A
            label = 'top'
        else:
            model = self.cb_bot_smu_model.currentText()
            addr  = self.le_bot_smu_addr.text().strip()
            try:
                compl = float(self.le_bot_smu_compl.text())
            except ValueError:
                compl = SMU_DEFAULT_COMPLIANCE_A
            label = 'bot'

        if not addr:
            self.log_event(f"{label} SMU test: no VISA address.", "error")
            return

        self.log_event(
            f"{label} SMU test: opening {model} @ {addr} (compliance "
            f"{compl*1e9:.0f} nA)...", "info")
        smu = KeithleyGateSource(label, model, addr, compliance_a=compl)
        try:
            smu.open()
            self.log_event(
                f"{label} SMU IDN: {smu._idn}", "success")
        except Exception as e:
            self.log_event(
                f"{label} SMU test FAILED: {e}", "error")
        finally:
            try:
                smu.close()
            except Exception:
    ...
    def _config_widgets(self):
        ...
        # v2.4: SMU model / address / compliance fields replace AO combos.
        widgets += [self.cb_top_smu_model, self.le_top_smu_addr,
                    self.le_top_smu_compl, self.btn_test_top_smu,
                    self.cb_bot_smu_model, self.le_bot_smu_addr,
                    self.le_bot_smu_compl, self.btn_test_bot_smu,
                    self.cb_fast_gate]
    ...
    def _parse_params(self):
        ...
        # ---------- 6) gate sources (Keithley SMUs, v2.4) ----------
        try:
            top_smu_model = self.cb_top_smu_model.currentText()
            top_smu_addr  = self.le_top_smu_addr.text().strip()
            top_smu_compl = float(self.le_top_smu_compl.text())
            bot_smu_model = self.cb_bot_smu_model.currentText()
            bot_smu_addr  = self.le_bot_smu_addr.text().strip()
            bot_smu_compl = float(self.le_bot_smu_compl.text())
        except ValueError:
            raise ValueError("SMU compliance current parsing failed (must "
                             "be a float in amps, e.g. 1e-7).")
        if not top_smu_addr:
            raise ValueError("Top SMU VISA address is empty.")
        if not bot_smu_addr:
            raise ValueError("Bottom SMU VISA address is empty.")
        if top_smu_addr == bot_smu_addr:
            raise ValueError("Top and bottom SMUs cannot share the same "
                             "VISA address.")
        if top_smu_compl <= 0 or bot_smu_compl <= 0:
            raise ValueError("SMU compliance current must be > 0.")
        if top_smu_compl > 1e-3 or bot_smu_compl > 1e-3:
            raise ValueError(
                "SMU compliance current > 1 mA refused as a safety floor "
                "for hBN-gated devices.  If you really need this, edit "
                "parse_params manually.")
        smu_config = dict(
            top_model=top_smu_model, top_addr=top_smu_addr,
            top_compliance_a=top_smu_compl,
            bot_model=bot_smu_model, bot_addr=bot_smu_addr,
            bot_compliance_a=bot_smu_compl,
        )
        # The symbolic gate channel names that propagate down to the
        # MeasurementThread, CompositeHardware, and CSV.  Always 'top'/'bot'.
        ao_top = CompositeHardware.AO_TOP
        ao_bot = CompositeHardware.AO_BOT
        ...
        # SMU range cap.  Both K2400 and K2636 can source >10 V, but
        # for hBN-gated devices the safety ceiling is ±20 V (above this
        # the dielectric is essentially guaranteed to fail).  Refuse to
        # let the user set anything above 20 V from the GUI.
        if vtg_max > 20.0 or vbg_max > 20.0:
            raise ValueError(
                "|Vtg|_max and |Vbg|_max cannot exceed 20 V.  hBN gate "
                "dielectric safety ceiling.")
    ...
    def _persistent_widgets(self):
        ...
        # v2.4: SMU fields replace the old AO combos.
        items.append(('combo',    'gates/top_smu_model', self.cb_top_smu_model))
        items.append(('lineedit', 'gates/top_smu_addr',  self.le_top_smu_addr))
        items.append(('lineedit', 'gates/top_smu_compl', self.le_top_smu_compl))
        items.append(('combo',    'gates/bot_smu_model', self.cb_bot_smu_model))
        items.append(('lineedit', 'gates/bot_smu_addr',  self.le_bot_smu_addr))
        items.append(('lineedit', 'gates/bot_smu_compl', self.le_bot_smu_compl))
            ...
            if self.thread and self.thread.isRunning():
                ...
                if not ok:
                    self.log_event(
                        ...
                        "HARDWARE STATE MAY BE INCONSISTENT.  Check SMU "
                        "front panels and manually turn OUTPUT OFF if "
                        "they are still on.", "error")
                    self.thread.terminate()
                    self.thread.wait(2000)
        finally:
            # v2.4: The worker already ramped gates and closed its SMUs
            # in its own finally block.  The GUI still owns the bare DAQ
            # (AI readout only — no AO channels to ramp) so we just need
            # to close it.
            try:
                self._save_settings()
            except Exception:
...
def main():
    ...
    if not DEMO_MODE:
        try:
            ...
            sys.exit(1)
        # v2.4: also need PyVISA for Keithley SMU communication.
        try:
            ...
            sys.stderr.write(
                ...
                f"v2.4 requires pyvisa for Keithley SMU communication.\n"
                f"Install with:  pip install pyvisa pyvisa-py\n"
                f"Or run with --demo to bypass hardware initialisation.\n")
            sys.exit(1)
